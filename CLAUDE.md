...
- **專案客戶**：分**廣告投放／顧問陪跑／專案協作**三區（客戶的 `group` 欄位，沒有這欄的舊資料一律當廣告投放，
  不需要搬移），每區是一排 4–5 個的小卡（手機 2 欄→640px 3 欄→980px 4 欄→1240px 5 欄）；
  已結案的另外收在最下面一區、不能拖。可自行新增／改簡稱／設認字關鍵字，每個可掛 Google 雲端資料夾連結。
