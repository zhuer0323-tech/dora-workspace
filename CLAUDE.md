...
### 內容
- **今日**：時鐘、今日聚焦、快速記一筆、每日例行、今日概覽、需要注意
  六張卡可拖曳排序、標題可點擊改名，順序與名稱都同步雲端
- **任務**：連動式儀表板 ＋ 清單／日／週／月／年 五種檢視，共用同一組篩選
- **工具**：釜山旅遊、IG 腳本、會議報告、報價單、數學遊戲
- **客戶**：30 個，可自行新增／改簡稱／設已結案／設認字關鍵字，每個可掛 Google 雲端資料夾連結
...
- **工作類型 9 種**：廣告（含上線／名單／調整／走期／社群／限動／文案／素材）、會議、提案、
  內部（只要提到「檢查」）、結案報告、報價單、KOL（kol／網紅／部落客）、週報回報（含「廣告回報」）、其他。
  比對規則：關鍵字愈長愈優先；**「內部」這一類整組最優先**（`kwTables()` 裡硬編，改名就失效）
- **舊類型自動併過去**：`migrateTypesV2()` 只跑一次（`ui.typesVersion === 2` 擋住），
  原本的類型留在任務的 `typeOld` 欄
